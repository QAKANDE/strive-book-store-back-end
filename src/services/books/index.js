...
const { readDB, writeDB , readComment } = require("../../utilities")
...
booksRouter.get("/:asin/comments" , async (req,res,next)=>{
    try {
        ...
        const commentBasedOnAsin = comments.filter(comment => comment.asin === req.params.asin)
        res.send(commentBasedOnAsin)
    } catch (error) {
        console.log(error)
    }
})
